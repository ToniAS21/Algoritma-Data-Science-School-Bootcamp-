...
```{r}
# code Here
cond1 <- retail$Profit > mean(retail$Profit)
cond2 <- retail$Quantity < 5
nrow(retail[cond1 & cond2,])
```
...
```{r}
# code here
cond2 <- retail$Category == "Office Supplies"

data_Off <- retail[cond2, ]  
# Dengan table
#data_Off <- table(data_Off$Category, data_Off$Sub.Category)
#data_Off
#data_Off[sort(data_Off, decreasing = T),]

agg_off <- 
aggregate(x=Quantity ~ Sub.Category, data = data_Off, FUN = sum)
agg_off[order(agg_off$Quantity, decreasing = T),]
```
...
abline(lm(retail$Sales ~ retail$Profit), col = "red")
```
...
- mean populasi = 47.858,35
- sd populasi = 25.195
- n = 1
- mean sampel = 81.777
- tingkat kepercayaan = 95%
- alpha = 5% -> 0.05 -> tingkat signifikansi

**Hitung Z-hitung & P-value**

```{r}
# cari SE = sd populasi / akar n
SE_promo <- 25195/sqrt(1)

# cari z-hitung = (mean populasi - mean sampel)/SE

z_promo <- (81777-47858.35)/SE_promo
z_promo
```
```{r}
# p-value 
# lower.tail = F karena dicari peluang data sampel kita ada di nilai ekstrim atas (berbeda signifikan dengan kebanyakan data normal)
p_value <-pnorm(z_promo, lower.tail = F)
p_value
```
**Bandingkan P-value dengan alpha**
NOTE: 
- p_value < alpha -> tolak H0 
- p_value > alpha -> gagal tolak H0

jawaban : p_value > alpha , gagal tolak HO

**Kesimpulan** :
Promosi tidak mempengaruhi jumlah penjualan
