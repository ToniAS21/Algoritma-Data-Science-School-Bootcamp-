...
```{r}
# your code here
us_kiva <- kiva[kiva$country == "United States",]
```
...
```{r}
# your code here
hist(us_kiva$lender_count)
```

```{r}
ggplot(data = us_kiva, aes(x = lender_count)) +
  geom_histogram()
```
...
```{r}
# your code here
manu_kiva <- kiva[kiva$sector == "Manufacturing",]
manu_kiva
```
...
```{r}
cor(manu_kiva$loan_amount, manu_kiva$lender_count)
```

```{r}
# your code here
plot(manu_kiva$loan_amount, manu_kiva$lender_count)
```
...
# Subset data khusus negara Philippines
phi_kiva <- kiva[kiva$country == "Philippines",]

# Mengubah tipe data kolom posted_time dan funded_time
phi_kiva$posted_time <- ymd_hms(phi_kiva$posted_time)
phi_kiva$funded_time <- ymd_hms(phi_kiva$funded_time)
phi_kiva
```
...
range(phi_kiva$posted_time)
```
```{r}
min(phi_kiva$posted_time)
max(phi_kiva$posted_time)
```
...
```{r}
# your code here
phi_kiva$funding_duration <- difftime(phi_kiva$funded_time, phi_kiva$posted_time, units = "hours")
```
...
```{r}
# your code here
phi_kiva$month <-  month(phi_kiva$posted_time, label = T, abbr = F)
phi_kiva$month <- as.factor(phi_kiva$month)
```
...
loan_agg[order(loan_agg$funding_duration, decreasing = T),]
```
