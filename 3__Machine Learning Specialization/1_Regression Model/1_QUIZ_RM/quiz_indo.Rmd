...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] the target variable is the monthly premium fee
...
```{r}
model_time <- lm(formula = crime_rate ~ time_prison, data = crime)
summary(model_time)
```
___
...
4. Manakah dari pernyataan berikut yang paling baik dalam menjelaskan *slope* (kemiringan) dari `time_prison`?
  ...
  - [X] Slope positif, dan secara statistik tidak signifikan (P-value lebih besar dari 0.05)
...
model_step <- step(object = model_crime_all,
                   direction = "backward",
                   trace = F
                   )
summary(model_step)
...
5. Berdasarkan *summary* dari model terbaik, pernyataan mana yang **tidak tepat**?
  ...
  - [X] Seluruh variabel prediktor yang digunakan pada model di atas signifikan mempengaruhi crime_rate
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] All variables used as predictor in the final model are significantly influence crime_rate
...
```{r}
crime_test$y_pred <- predict(object = model_step, newdata = crime_test)
```
...
```{r}
MAE(y_pred = crime_test$y_pred, y_true = crime_test$crime_rate)
```
___
