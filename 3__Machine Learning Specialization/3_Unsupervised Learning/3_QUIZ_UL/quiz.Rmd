...
5. Manakah dari pernyataan berikut ini yang **TIDAK BENAR** tentang PCA?
 ...
 - [X] Kita tidak dapat merekonstruksi data awal kita secara sempurna dari hasil PCA meskipun kita memiliki eigen value dan eigen vector
...
6. Manakah dari pernyataan berikut yang **TIDAK BENAR** tentang pemilihan jumlah cluster?
  ...
  - [X] Jumlah cluster yang banyak selalu mengindikasikan cluster yang baik
...
*Referensi Opsi Bahasa Inggris:*  
  ...
  - [X] A high number of clusters always indicates a good clustering
...
7. Manakah dari pernyataan berikut ini yang **TIDAK BENAR** tentang K-Means?
  ...
  - [X] Semakin tinggi nilai `betweenss` mengindikasikan variansi data yang tinggi di dalam setiap cluster
  
Betweenss gede bagus tetapi perlu di normalisasi dlu dibagi tots  
...
```{r}
# your code here
fruits_clean["Apricot",]
```

```{r}
fruits_clean %>% filter(cluster == 1)
```
...
9. Bisakah Anda mendeskripsikan karakteristik buah yang ada di cluster yang sama dengan `Mango`!
  ...
  - [X] Nilai rata-rata protein terendah, rata-rata kandungan air (*water*) tertinggi, dan rata-rata vitamin_a_iu tertinggi
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] Lowest mean of protein, highest mean of water, and highest mean of vitamin_a_iu
