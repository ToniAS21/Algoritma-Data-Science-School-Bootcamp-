...
- [X] the more appropriate model to use is the k-nearest neighbor
...
turnover %>% 
  filter(left == 1) %>% 
  group_by(division) %>% 
  summarise(mean = mean(average_monthly_hours)) %>% 
  arrange(-mean)
```
___
2. Berdasarkan agregasi data yang telah Anda buat, divisi mana yang merupakan 3 divisi dengan rata-rata jam kerja per bulan tertinggi?
  ...
  - [X] Technical, IT, Research and Development (RandD)
___
...
train_turnover <- turnover[index,]
test_turnover <- turnover[-index,]
```
...
```{r}
# your code here
prop.table(table(train_turnover$left)) %>% round(2)
prop.table(table(test_turnover$left)) %>% round(2)
```
...
3. Berdasarkan hasil di atas, manakah pernyataan di bawah ini yang paling tepat?
  ...
  - [X] Distribusi dikatakan seimbang, namun kita juga harus menguji apakah proporsi kelas pada data test dapat dikatakan seimbang.
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] The class distribution is balanced, but we should also make sure that the test data set also have balanced proportion.
...
```{r}
model_logistic <- glm(formula = left ~ ., data = train_turnover, family = "binomial")
```
...
Menemukan odds (Perbandingan probilitas)
```{r}
exp(1.5014787)
```

```{r}
levels(train_turnover$salary)
```
...
4. Regresi logistik adalah salah satu model yang dapat diinterpretasikan. Kita dapat menjelaskan bagaimana masing-masing variabel memberikan pengaruh pada hasil prediksi. Berdasarkan ringkasan model di atas, apa yang dapat diinterpretasikan dari koefisien `salarymedium`?
  ...
  - [X] Karyawan yang mendapatkan medium salary 4.48 kali lebih mungkin untuk mengundurkan diri dibandingkan karyawan yang menerima high salary.
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] Employee who received medium salary is about 4.48 more likely to resign than the employee who received high salary.
...
train_x <- train_turnover %>% select_if(is.numeric)

# variabel prediktor pada `test`
test_x <- test_turnover %>% select_if(is.numeric)

# variabel target pada `train`
train_y <- train_turnover %>% select(left)

# variabel target pada `test`
test_y <- test_turnover %>% select(left)
```
...
*Referensi Opsi Bahasa Inggris:*
  - [ ] use k = 5
  - [ ] number of row
  - [X] square root by number of row 
___
...
```{r}
prob_value <- predict(object = model_logistic, newdata = test_turnover, type = "response")
```
...
loans_test$pred.Label <- ifelse(loans_test$pred.Risk>0.5 , 1 , 0) %>% as.factor()

```{r}
pred_value <- ifelse(prob_value>0.55, 1, 0) %>% as.factor()
```
...
confusionMatrix(data=pred_value, reference=test_turnover$left, positive="1")
```
...
9. Sekarang, ingat apa yang sudah kita pelajari mengenai kelebihan dan kekurangan masing-masing model. Manakah pernyataan di bawah ini yang **TIDAK TEPAT**?
  ...
  - [X] Masih lebih baik menggunakan kNN daripada Logistic Regression untuk mendapatkan performa model yang lebih tinggi, meskipun sebagian besar prediktor adalah variabel kategorik
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] It is still better to use kNN than Logistic Regression to gain higher model performance, even when most of your predictor is categorical variables.
