...
set.seed(100)
# your code here
data_train_down <- downSample(data_train %>% select(-default),
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] Assume that between target and predictor variables are independent
  - [ ] Skewness due to data scarcity
___
...
```{r}
# your code here
pred_naive <- predict(model_naive, data_test$default, type = "class")
```
...
3. Pada model decision tree, tujuan dari mengatur `mincriterion = 0.90` adalah ...
  ...
  - [ ] Untuk pemangkasan (*pruning*) pohon, hanya memperbolehkan pohon yang memiliki maksimum 10% data pada *terminal node*. -> minbucket

*Referensi Opsi Bahasa Inggris:*
  - [ ] To prune our model, we let the tree that has p-value <= 0.10 to split the node
  - [X] To prune our model, we let the tree that has p-value <= 0.90 to split the node
  - [ ] To prune our model, we let the tree that has a maximum of 10% of the data in the terminal nodes

___

https://meet.google.com/unm-mmzs-vxg
...
```{r}
# your code here
pred_dt <- predict(object = model_dt, data_test, type = "response")
```
...
*Referensi Opsi Bahasa Inggris:*
  - [X] We have 33.61% error of our unseen data
...
```{r}
# your code here
pred_rf <- predict(object = model_rf, data_test, type = "raw")
```
...
```{r}
# your code here
perf <- performance(prediction.obj = pred_roc,
                    measure = "tpr",
                    x.measure = "fpr")
```
...
```{r}
# your code here
auc <- performance(prediction.obj = pred_roc,
                   measure = "auc")
auc@y.values
```
...
```{r}
nv <- confusionMatrix(data = pred_naive, reference = data_test$default, positive = "yes")

dt <- confusionMatrix(data = pred_dt, reference = data_test$default, positive = "yes")

rf <- confusionMatrix(data = pred_rf, reference = data_test$default, positive = "yes")
```
