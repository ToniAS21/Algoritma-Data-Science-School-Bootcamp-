...
assault %>% head() %>% arrange()
```


```{r}
assault_ts <- ts(dat = assault$Amount_Assault, start = 2014, frequency = 365)
class(assault_ts)
```
___

1. Data time series memiliki beberapa karakteristik. Manakah pernyataan di bawah ini yang **BENAR** terkait data time series?
  - [X] *Additive time series* disebut additive karena variasi *seasonal*-nya relatif konstan dari waktu ke waktu
...
*Referensi Opsi Bahasa Inggris:*
 - [X] Additive time series is additive because the seasonal variation is relatively constant over time
...
```{r}
# your code here
decompose(assault_ts, type = "additive") %>% 
  autoplot()
```
___
...
train_assault <- head(assault_ts, -length(test_assault))
```
...
```{r}
# your code here
model_hw <- HoltWinters(x = train_assault, seasonal = "additive")
```
...
5. Berdasarkan penjelasan di atas, manakah dari pernyataan dibawah ini yang **BENAR** mengenai ARIMA(p,d,q)?
  ...
  - [X] q menunjukkan berapa banyak data yang digunakan untuk melakukan *smoothing error* menggunakan *Moving Average*
...
*Referensi Opsi Bahasa Inggris:*
 ...
 - [X] q shows the amount of data for smoothing error using Moving Average
___
...
6. Berdasarkan hasil di atas, manakah dari pernyataan berikut yang **BENAR**?
  ...
  - [X] Perbedaan nilai *mean absolute percentage error* antara model ARIMA dan *Holt-Winters* adalah 1.59%
...
*Referensi Opsi Bahasa Inggris:*
 ...
 - [X] The difference of mean absolute percentage error between Holt-Winters and Holt-Winters model is 1.59%
___
...
> Gagal Tolak Ho karena nilai p-value > alpha

___
8. Manakah dari pernyataan berikut yang **BENAR** mengenai asumsi *no-autocorrelation* pada model *time series* kita?
  - [X] tidak terdapat *autocorrelation* pada *error*, artinya setiap *error* tidak memiliki hubungan
...
*Referensi Opsi Bahasa Inggris:*
 - [X] there is no autocorrelation in error, meaning that each error have no relation
