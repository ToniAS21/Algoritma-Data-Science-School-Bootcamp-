...
```{r}
# your code here
unique(sign_train$label) %>% sort()
unique(sign_test$label) %>% sort()
```
...
train_x <- sign_train %>% select(-label) %>% as.matrix() / 255

# Predictor variables in `sign_test`
test_x <- sign_test %>% select(-label) %>% as.matrix() / 255

# Target variable in `sign_train`
train_y <- sign_train %>% select(label)

# Target variable in `sign_test`
test_y <- sign_test %>% select(label)
```
...
1. Jika Anda mengecek suatu gambar di dalam data train, kita akan melihat bahwa nilai pixel jatuh dalam rentang 0 sampai 255. Kemudian, apakah tujuan dari membagi nilai pada array tersebut dengan 225?
  ...
  - [X] Menormalisasi rentang nilai yang sebelumnya 0 hingga 255 menjadi 0 hingga 1
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] Normalize the array value from 0 to 255 into 0 to 1
...
train_y_dummy <- to_categorical(train_y$label, num_classes = 24)
test_y_dummy <- to_categorical(test_y$label, num_classes=24)
```
...
2. Manakah dari pernyataan berikut ini yang **TIDAK BENAR** tentang Neural Network?
  ...
  - [X] *Activation function* tidak melakukan transformasi data
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] Activation function is not doing any transformation to the data
___
...
model_base <- keras_model_sequential(name = "model_base") %>% 
  layer_dense(units = 64, activation = "relu", input_shape = 784, name = "hidden_1") %>% 
  layer_dense(units = 32, activation = "relu", name = "hidden_2") %>% 
  layer_dense(units = 24, activation = "softmax", name = "output")
```
...
model_bigger <- keras_model_sequential(name = "model_bigger") %>% 
  ...
  layer_dense(units = 64, activation = "relu", name = "hidden_3") %>% 
  layer_dense(units = 24, activation = "softmax", name = "output")
```
...
```{r}
# your code here
model_base %>% compile(loss = loss_categorical_crossentropy(),
...
```{r}
# your code here
model_bigger %>% compile(loss = loss_categorical_crossentropy(),
                         optimizer = optimizer_adam(learning_rate = 0.001),
                         metrics = "accuracy")
```   
...
```{r}
# your code here
history_base <- model_base %>% fit(x = train_x, 
                                   y = train_y_dummy, 
                                   validation_data = list(test_x, test_y_dummy),
                                   epoch = 15, 
                                   batch_size = 4200, 
                                   shuffle = F)
plot(history_base)
...
```{r}
# your code here
history_bigger <- model_bigger %>% fit(x = train_x,
                                       y = train_y_dummy,
                                       validation_data = list(test_x, test_y_d),
                                       epoch = 15, 
                                       batch_size = 4200,
                                       shuffle = F)
plot(history_bigger)
...
```{r}
# your code here
pred_base <- predict_classes(model_base, test_x_array)

pred_bigger <- predict_classes(model_bigger, test_x_array)
```
...
```{r}
# your code here
confusionMatrix(data = as.factor(pred_base), reference = as.factor(test_y$label))
confusionMatrix(data = as.factor(pred_bigger), reference = as.factor(test_y$label))
```
...
5. Dari dua confusion matrix di atas, pernyataan di bawah yang paling sesuai adalah?
  ...
  - [X] Semakin banyak hidden layer dan neuron, performa model dapat meningkat karena semakin banyak features yang dapat diekstraksi dari data
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] The more hidden layer and neuron, the model may have better performance because more features will be extracted from the data
...
model_tuning <- keras_model_sequential(name = "model_tuning") %>% 
  layer_dense(units = 128, activation = "relu", input_shape = 784, name = "hidden_1") %>% 
...
```{r}
# your code here
model_tuning %>% compile(loss = loss_categorical_crossentropy(),
                         optimizer = optimizer_adam(learning_rate = 0.001),
                         metrics = "accuracy")
```
...
```{r}
# your code here
history_tuning <- model_tuning %>% fit(x = train_x,
                                       y = train_y_dummy,
                                       validation_data = list(test_x, test_y_dummy),
                                       epoch = 10, 
                                       batch_size = 150,
...
```{r}
# your code here
pred_tuning <- predict_classes(model_tuning, test_x_array)
```
...
```{r}
# your code here
confusionMatrix(data = as.factor(pred_tuning), reference = as.factor(test_y$label))
```
...
6. Hidden layer merupakan tempat dimana informasi dari data diekstraksi. Apa yang dapat Anda simpulkan dari `model_bigger` dan `model_tuning` mengenai hidden layer?
  ...
  - [X] Menggunakan jumlah hidden layer yang relatif sedikit maupun banyak tidak mempengaruhi performa model
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] Using both of small or large number of hidden layers does not influence the model's performance
...
7. Dari 3 model yang telah dibuat (`model_base`, `model_bigger` dan `model_tuning`), model mana yang paling baik untuk kita pilih?
  ...
  - [X] *model_tuning*, karena akurasinya cukup tinggi dan selisih akurasi antara data train dan data test paling kecil
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] *model_tuning*, because the accuracy is quite high and the difference in accuracy between train data and test data is the smallest
...
8. Dibawah ini, manakah dari pasangan kasus dan data yang **harus** dianalisis menggunakan model deep learning?
  ...
  - [X] Pembuatan *speech recognizer*; data yang tersedia berupa set file suara yang direkam oleh orang-orang
...
*Referensi Opsi Bahasa Inggris:*
  ...
  - [X] Creating a speech recognizer; available data includes sound files recorded by people
